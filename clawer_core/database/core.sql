...
drop table T_EXTEND_CODE cascade constraints;
...
create table T_EXTEND_CODE (id number(19,0) not null, CODE_ID number(19,0), CODE_TYPE varchar2(10 char) not null,CODE varchar2(50 char) not null, NAME varchar2(100 char), memo varchar2(250 char), UPDATE_DATE timestamp, UPDATE_USER varchar2(50 char), primary key (id), unique (CODE_TYPE, CODE));
...
alter table T_EXTEND_CODE add constraint FK_CODE_EXTEND foreign key (CODE_ID) references T_CODE;
...
create sequence SEQ_EXTEND_CODE START WITH 100000;
...
grant all on core.T_EXTEND_CODE to baixing;
...
grant all on core.T_MAIL_SERVER to baixing;

grant all on core.seq_code  to baixing;
grant all on core.seq_code_mapping  to baixing;
grant all on core.seq_extend_code  to baixing;
grant all on core.seq_mail_senders  to baixing;
