...
public class ImageScale {
	...
	public BufferedImage imageZoomOut(BufferedImage srcBufferImage, float w, float h) {
		...
		BufferedImage pbFinalOut = VerticalFiltering(pbOut, (int) h);
		return pbFinalOut;
	}
	 ...
	 * 决定图像尺寸
	 */
	private int DetermineResultSize(float w, float h) {
	...
	private double Lanczos(int i, float inWidth, float outWidth, double Support) {
		double x;
	...
	// 图片水平滤波
	private BufferedImage HorizontalFiltering(BufferedImage bufImage, float iOutW) {
		int dwInW = bufImage.getWidth();
		int dwInH = bufImage.getHeight();
		int value = 0;
		BufferedImage pbOut = new BufferedImage((int) iOutW, dwInH, BufferedImage.TYPE_INT_RGB);
