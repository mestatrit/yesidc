...
//    ); 
   ...
   for (int rowIndex = 0; rowIndex < readonlySheet.getRows(); rowIndex++) { 
    //@TODO writableSheet.setRowView(rowIndex,readonlySheet.getRowView(rowIndex)); 
   } 
...
// TODO  labelB.setCellFeatures(cellFeatures); 
...
public static void drawRect(WritableSheet sheet, int x, int y, int width, int height,BorderLineStyle style,Colour BorderColor, Colour bgColor) throws WriteException { 
   for(int w = 0; w < width; w++){ 
    for(int h = 0; h < height; h ++) { 
     ...
     if(h == 0)//画上 
    	// TODO  alignStyle.setBorder(Border.TOP, style, BorderColor);//设置边框的颜色和样式 

     if(w == 0)//画左 
    	// TODO  alignStyle.setBorder(Border.LEFT, style, BorderColor);//设置边框的颜色和样式 

     if(w == width - 1)//画右 
    	// TODO  alignStyle.setBorder(Border.RIGHT, style, BorderColor);//设置边框的颜色和样式 

     if(h == height -1)//画下 
    	// TODO alignStyle.setBorder(Border.BOTTOM, style, BorderColor);//设置边框的颜色和样式 
