...
public class TestCompanyDao extends TestCase {
	...
	public static CompanyDao companyDao = (CompanyDao) SpringContext
			.getBean("companyDao");

	static String str = "'<DIV class=\"jobs_txt\"> <p class=\"txt_font\">摩卡软件有限公司（Mocha Software Co.,Ltd.）"
			+ "随着公司业务的迅猛发展，目前已成为亚太地区最大的软件产品和解决方案提供商之一，我们为客户"
			+ "提供创新的产品、专业的IT解决方案和优质的服务。<br>摩卡软件有限公司，下设近10个事业部。以先"
			+ "进的研发和技术理念，以及求知向上的工作氛围吸引着无数国、内外优秀的行业技术精英——博士、硕士"
			+ "、学士学位的高素质人才600余名，在摩卡实验室，汇聚了一批具有国外研发经验的架构师、设计师，基于"
			+ "J2EE架构设计的摩卡五大产品，包括流程管理、监控软件、搜索引擎、以及手机办公等无线应用领域产品"
			+ "。目前，摩卡软件有限公司自主研发的品牌软件—摩卡产品，已经在中国移动、中国联通以及交通银行"
			...
			+ "<br>作为一家外资企业，摩卡软件在引入国际先进的管理理念的同时，提出被员工高度认同的核心价"
			+ "值观，建立优秀的企业文化。我们的核心价值观是：“以客户为中心；高质量的产品、优质的服"
			+ "务；团结就是力量；不断创新、追求完美。”<br>现诚邀有志之士加盟，公司将为您提供公"
			+ "平开放的工作环境，完善的福利政策，广阔的职业发展空间，科学高效全面基于价值的薪酬和"
...
//		for(Iterator<ComContactHeader> it = comContactHeaders.iterator();it.hasNext();){
//			cchd = it.next();
...
//			ces.add(ce);
	...
	private static void test1() {
		try {
			...
			comAppends.add(ca);
			System.out.print("companyInfoHandlerService:"
					+ companyInfoHandlerService.toString());
			companyDao.save(c);
		} catch (Exception e) {
			e.printStackTrace();
		}
