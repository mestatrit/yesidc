...
public class WebrendererContext {
	...
	static {
		...
		BrowserFactory.setLicenseData(EmailConstants.WEBRENDERER_ID, EmailConstants.WEBRENDERER_SN);
		log.info("WebrendererContext Load license OK! Time is " + (System.currentTimeMillis() - l));
	...
	public synchronized static void reFreshContext(int index, String title) {
		...
		try {
			...
			log.error(title + " Destory Browser Error!", e);
		}
		wre.setBrowser(null);

		IBrowserCanvas browser = BrowserFactory.spawnMozilla();
		browser.enableImageLoading(false);
		WebrendererSupport.addListener(browser, EmailConstants.DIALOG_USERNAME, EmailConstants.DIALOG_PASSWORD);
		wre.setLoaded(true);
	...
	public static Map<Integer, WebRenderEntity> initContext() {
		...
		for (int i = 0; i < EmailConstants.THREADS_NUMBER + 1; i++) {
			IBrowserCanvas browser = BrowserFactory.spawnMozilla();
			browser.enableImageLoading(false);
			WebrendererSupport.addListener(browser, EmailConstants.DIALOG_USERNAME, EmailConstants.DIALOG_PASSWORD);
		...
		log.info("Initial " + EmailConstants.THREADS_NUMBER + " BROWSERS OK!");
